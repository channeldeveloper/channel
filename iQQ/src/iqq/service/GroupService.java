...
public class GroupService {
    ...
    public List<Group> downloadGroupList(AuthInfo ai) {
        if (groupList != null) {
            return groupList;
        }
        ...
        try {
            ...
            String content = "{\"vfwebqq\":\"" + ai.getVfwebqq() + "\"}";
            try {
            ...
            if (retJson.getInt("retcode") == 0) {
                ...
                for (int i = 0; i < array.length(); i++) {
                    ...
                    this.downloadImage(ai, group, path + group.getFlag() + ".jpg");
    ...
    public void downloadImage(AuthInfo ai, Group group, String path) {
        ...
        if (intervals > 3600000) {
            try {
                String urlStr = "http://face1.qun.qq.com/cgi/svr/face/getface?cache=0&type=4&fid=0&uin=" + group.getCode() + "&vfwebqq=" + ai.getVfwebqq();
    ...
    public List<Member> downloadMemberList(AuthInfo ai, long code) {
        Log.println("-----------下载群成员-------------");
        ...
        try {
            String urlStr = "http://s.web2.qq.com/api/get_group_info_ext2?gcode=" + code + "&vfwebqq=" + ai.getVfwebqq() + "&t=" + System.currentTimeMillis();
    ...
    public Group getNumberForCode(AuthInfo ai, long code) {
        if (groupList != null) {
            for (Group g : groupList) {
                if (g.getCode() == code) {
                    if (g.getNumber() == -1) {
                        g.setNumber(downloadNumberForCode(ai, code));
                    }
                    return g;
                }
            }
    ...
    public void fillNumber(final AuthInfo ai) {
        Runnable r = new Runnable() {

            @Override
            public void run() {
                for (Group group : groupList) {
                    group.setNumber(downloadNumberForCode(ai, group.getCode()));
                }
            }
        };
    ...
    public long downloadNumberForCode(AuthInfo ai,long code) {
        long groupNumber = 0;
        try {
            String urlStr = "http://s.web2.qq.com/api/get_friend_uin2?tuin=" + code + "&verifysession=&type=4&code=&vfwebqq=" + ai.getVfwebqq() + "&t=" + System.currentTimeMillis();
            httpService = new HttpService(urlStr, Method.GET);
            String result = httpService.sendHttpMessage();
            JSONObject retJson = new JSONObject(result);
            if (retJson.getInt("retcode") == 0) {
