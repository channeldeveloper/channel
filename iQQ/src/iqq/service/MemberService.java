...
public class MemberService {
    ...
    public ImageIcon downloadFace(AuthInfo ai, long uin) throws Exception {
        return downloadFace(ai, this.get(ai, uin));
    }

    public ImageIcon downloadFace(AuthInfo ai, Member member) throws Exception {
    ...
    public void downloadImage(AuthInfo ai, Member member, final String path) {
        try {
            final String urlStr = "http://face10.qun.qq.com/cgi/svr/face/getface?cache=1&type=1&fid=0&uin=" + member.getUin() 
            		+ "&vfwebqq=" + ai.getVfwebqq() + "&t=" + System.currentTimeMillis();
    ...
    public Member getQQLevel(AuthInfo ai, Member member) throws Exception {
        if (member.getLevel() != null && !member.getLevel().equals("")) {
            return member;
        }
        String urlStr = "http://s.web2.qq.com/api/get_qq_level2?tuin=" + member.getUin() + "&vfwebqq=" + ai.getVfwebqq() + "&t=" + System.currentTimeMillis();
    ...
    public Member getSignature(AuthInfo ai, Member member) throws Exception {
        //Member member = Auth.getMember();
        if (member.getLnick() != null && !member.getLnick().equals("")) {
            return member;
        }
        String urlStr = "http://s.web2.qq.com/api/get_single_long_nick2?tuin=" + member.getUin() + "&vfwebqq=" + ai.getVfwebqq() + "&t=" + System.currentTimeMillis();
        httpService = new HttpService(urlStr, Method.GET);
        String result = httpService.sendHttpMessage();
        JSONObject retJson = new JSONObject(result);
        if (retJson.getInt("retcode") == 0) {
        ...
        return getQQLevel(ai, member);
    }

    public Member getMemberInfo(AuthInfo ai, Member member) throws Exception {
        if (member.getLnick() == null || member.getLnick().equals("")) {
            member = getSignature(ai, member);
        }
        //String urlStr = "http://s.web2.qq.com/api/get_self_info2?t=1341188224874";
        String urlStr = "http://s.web2.qq.com/api/get_friend_info2?tuin=" + member.getUin() + "&verifysession=&code=&vfwebqq=" + ai.getVfwebqq() + "&t=" + System.currentTimeMillis();
        httpService = new HttpService(urlStr, Method.GET);
        String result = httpService.sendHttpMessage();
        JSONObject retJson = new JSONObject(result);
        if (retJson.getInt("retcode") == 0) {
...
//            }
     ...
     public Member getMemberAccount(Member member) throws Exception {
    	return getMemberAccount(null, member);
    }

    public Member getMemberAccount(AuthInfo ai, Member member) throws Exception {
        ...
        try {
            String urlStr = "http://s.web2.qq.com/api/get_friend_uin2?tuin=" + member.getUin() + "&verifysession=&type=1&code=&vfwebqq=" + ai.getVfwebqq() + "&t=" + System.currentTimeMillis();
            httpService = new HttpService(urlStr, Method.GET);
            String result = httpService.sendHttpMessage();
            JSONObject retJson = new JSONObject(result);
            if (retJson.getInt("retcode") == 0) {
    ...
    public boolean changeStatus(AuthInfo ai, String status) {
        if (status == null || status.trim().equals("")) {
            return false;
        }
        Log.println("改变状态:" + status);
        try {
            String urlStr = "http://d.web2.qq.com/channel/change_status2?newstatus=" + status + "&clientid=" + ai.getClientid() + "&psessionid=" + ai.getPsessionid() + "&t=" + System.currentTimeMillis();
            httpService = new HttpService(urlStr, Method.GET);
            String result = httpService.sendHttpMessage();
            JSONObject retJson = new JSONObject(result);
            if (retJson.getInt("retcode") == 0) {
