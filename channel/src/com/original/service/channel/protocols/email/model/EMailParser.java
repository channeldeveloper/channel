...
//                System.out.println("disposition:" + disposition);
                  ...
                  // System.out.println("headers:" + mpart.getAllHeaders());
...
//                处理内嵌图片的附件问题，dispose可能为null
					...
					if (contype.toLowerCase().indexOf("application") != -1
						...
						if(cid != null && cid.length > 0)
						{
							 ...
							 String contentId = mpart.getHeader("Content-ID")[0].replace("<", "").replace(">", "");
//						  if (mpart.getHeader("Content-ID") != null) {
//		                        attachment.setCId(mpart.getHeader("Content-ID")[0].replace("<", "").replace(">", ""));
//		                    }
							String fileName = contentId;//MailParseUtil.getMessageFileName(mpart.getFileName());
		                    String extention = null;
		                    if (fileName.contains(".")) {
		                        extention = fileName.substring(fileName.lastIndexOf(".") + 1);
		                        attachment.setFileName(fileName);
